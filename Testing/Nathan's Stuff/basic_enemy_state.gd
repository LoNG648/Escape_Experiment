...
func _physics_process(delta):
	...
	if velocity.x > 0:
		$Sprite.flip_h = false
		#if detection_box.position.x < 0:
			#detection_box.position.x = abs(detection_box.position.x) * -1
		#if hurtbox_collision.position.x < 0:
			#hurtbox_collision.position.x = abs(hurtbox_collision.position.x) * -1
	elif velocity.x < 0:
		$Sprite.flip_h = true
		#if detection_box.position.x > 0:
			#detection_box.position.x = abs(detection_box.position.x) * -1
		#if hurtbox_collision.position.x > 0:
			#hurtbox_collision.scale.x = abs(hurtbox_collision.position.x) * -1
	if velocity.x != 0:
